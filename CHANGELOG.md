...
back on every run, which the Docker entrypoint already does at each start.
Without `?ordering=`, `GET /api/controls/` follows the places.
...
- **The control register listed A.5.10 before A.5.2.** Controls sorted by
  ...
  and PCI DSS read 12.1, 12.10, 12.2, while the folder tree beside them already
  read A.5.2 before A.5.10. Each control now keeps its place in its
  framework's file, the way categories already did, and the register, a
  framework's control list, the controls CSV, the evidence-link picker, the
  responsibility matrix and a vendor's shared-responsibility matrix follow it.
  A control added with no place goes last in its category.
