...
[![Release](https://img.shields.io/badge/release-v0.9.5g-1D6FE0.svg)](CHANGELOG.md)
...
with its version (`0.9.5g`), with the first seven characters of the commit it
...
```bash
docker pull ghcr.io/dboudreau00/conformiti-backend:0.9.5g
docker compose -f docker-compose.yml -f docker-compose.ghcr.yml pull
CONFORMITI_VERSION=0.9.5g docker compose -f docker-compose.yml -f docker-compose.ghcr.yml up -d
```
...
git fetch --tags && git checkout v0.9.5g
...
scripts/backup.sh
git fetch --tags && git checkout v0.9.5g
CONFORMITI_VERSION=0.9.5g docker compose -f docker-compose.yml -f docker-compose.ghcr.yml pull
CONFORMITI_VERSION=0.9.5g docker compose -f docker-compose.yml -f docker-compose.ghcr.yml up -d
```
