...
[![Release](https://img.shields.io/badge/release-v0.9.5m-1D6FE0.svg)](CHANGELOG.md)
...
with its version (`0.9.5m`), with the first seven characters of the commit it
...
```bash
docker pull ghcr.io/dboudreau00/conformiti-backend:0.9.5m
CONFORMITI_VERSION=0.9.5m docker compose -f docker-compose.yml -f docker-compose.ghcr.yml pull
CONFORMITI_VERSION=0.9.5m docker compose -f docker-compose.yml -f docker-compose.ghcr.yml up -d
```
...
git fetch --tags && git checkout v0.9.5m
...
# In .env, move the pin to the new release: CONFORMITI_VERSION=0.9.5m
docker compose pull && docker compose up -d
```

Change the pin in `.env`, not on the command line. A version given inline
(`CONFORMITI_VERSION=0.9.5m docker compose ...`) or exported lasts for that
